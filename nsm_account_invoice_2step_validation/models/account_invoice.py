...
class AccountInvoice(models.Model):
            ...
            if 'invoice_line_ids' in vals:
                if inv.type in ('in_invoice', 'in_refund'):
                    ...
                    if invoice_line.analytic_account_id:
                        team_acc_mapp = self.env['sales.team'].search([('analytic_account_id','=',invoice_line.analytic_account_id.id)], limit=1)
                        if team_acc_mapp and team_acc_mapp.sales_team_id:
                            inv.team_id = team_acc_mapp.sales_team_id.id
        return res

    # need to check w.r.t V14
    # @api.depends('invoice_line_ids.price_subtotal','invoice_line_ids.analytic_account_id.overhead_costs', 'currency_id', 'company_id', 'date_invoice', 'type')
    ...
    #                 if line.analytic_account_id.overhead_costs == True:
